...
c        dimension BCinp(numpbc,ndofBC2)
c  
      dimension BCtmp(nshg,ndofBC2)  
c
c.... --------------------------->  Input  <---------------------------
c
...
c
c output:
c  BCtmp    (nshg,ndofBC2) : The constraint data for prescribed BC 
c
c
c----------------------------------------------------------------------
c
...
c
     ...
     &            x(numnp,nsd),             ilwork(nlwork)
c
      dimension  BCtmpSAV(nshg,ndofBC2)
      dimension  BCtmp(nshg,ndofBC2),      fBC(nshg,ndofBC),
     &            e1(3),                    e2(3),
     &            elnrm(3),                 asum(numnp)
c
      integer sid, nsidg
...
c
      ...
      real*8 x(numnp,nsd),BCtmp(nshg,ndofBC2)
